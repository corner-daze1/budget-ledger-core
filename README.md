...
`miniprogram/` 是原生微信小程序壳，页面只通过 `src/application/app-core.js` 使用已验证的领域能力；应用层不接网络或 AI。当前固定四页依次为账本、我的（设置与资产）、统一“支出/收入/转账”入口和账单分析，数据通过微信本地存储保存版本化 JSON。底部视觉导航为“账本｜记一笔｜我的”，其中只有账本和我的是 Tab，中央“记一笔”是动作入口；完整账单从账本页“全部”进入。已初始化用户默认进入账本，首次无账本时自动转到我的完成设置。初始化后的我的页使用“预算与周期、计划提醒、资产账户、数据与隐私”四个互斥折叠分区。
